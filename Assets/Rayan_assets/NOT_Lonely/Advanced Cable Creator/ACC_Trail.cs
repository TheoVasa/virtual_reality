namespace NOT_Lonely
{
    ...
    public class ACC_Trail : MonoBehaviour
    {
        ...
        [Header("Collision Draping (per-segment)")]
        [Tooltip("Raycast down from each segment and snap it to the hit surface.")]
        public bool enableCollisionDraping = false;
        [Tooltip("How high above the segment to start the raycast.")]
        public float raycastHeight = 1f;
        [Tooltip("Offset above the hit point so the cable doesn’t clip.")]
        ...
        public void UpdateSequencesPositions()
        {
            ...
            for (int s = 0; s < cableSequences.Count; s++)
            {
                ...
                for (int c = 0; c < cableSequences[s].cables.Count; c++)
                {
                    ...
                    if (enableCollisionDraping)
                    {
                        RaycastHit hit;

                        // Only apply draping if NOT the last control point (so the plug stays free)
                        if (c < controlPointsStart.Count - 1)
                        {
                            Vector3 origin = posStart + Vector3.up * raycastHeight;
                            if (Physics.Raycast(origin, Vector3.down, out hit, raycastHeight * 2f))
                                posStart.y = hit.point.y + collisionOffset;

                            origin = posEnd + Vector3.up * raycastHeight;
                            if (Physics.Raycast(origin, Vector3.down, out hit, raycastHeight * 2f))
                                posEnd.y = hit.point.y + collisionOffset;
                        }
                    }
