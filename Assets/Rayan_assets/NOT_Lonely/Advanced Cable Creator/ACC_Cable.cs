namespace NOT_Lonely
{
    ...
    public class ACC_Cable : MonoBehaviour
    {
        ...
        public class Vector3Comparer : IEqualityComparer<Vector3>
        {
            ...
            {
                // Применяем коэффициент и округление для создания хэш-кода

                int hashX = Mathf.RoundToInt(obj.x / tolerance);
                int hashY = Mathf.RoundToInt(obj.y / tolerance);
                int hashZ = Mathf.RoundToInt(obj.z / tolerance);

                // Используем 23 и 47 как простые числа для уникального комбинирования хэшей
        ...
        //Optimization contributed by Minseo Han. Call ReDraw at the end of the frame in LateUpdate
...
*/
            ...
            //Optimization contributed by Minseo Han
            Vector3[] normals = mesh.normals;
            // Optimize: Store (normal vector sum, count) for each vertex group
            Dictionary<Vector3, (Vector3 normalSum, int count)> vertexGroups = new(new Vector3Comparer());
            // Iterate through vertices to sum normal vectors and count
            for (int i = 0; i < meshVertices.Length; i++) {
                Vector3 vertex = meshVertices[i];
                if (!vertexGroups.ContainsKey(vertex)) {
                    vertexGroups[vertex] = (normals[i], 1);
                } else {
                    var (normalSum, count) = vertexGroups[vertex];
                    vertexGroups[vertex] = (normalSum + normals[i], count + 1);
                }
            }
            // Calculate and apply averaged normal vectors
            for (int i = 0; i < meshVertices.Length; i++) {
                var (normalSum, count) = vertexGroups[meshVertices[i]];
                normals[i] = normalSum / count;
            }
            mesh.normals = normals;
            //
